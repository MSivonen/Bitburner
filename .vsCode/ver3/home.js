...
export async function main(ns) {
	...
	function getTargets() {
		...
		targetObjectArray = [];
		for (let i = 0; i < servers.length; i++) {
			if (ns.getServerMaxMoney(servers[i]) > 1000 && servers[i] != "home" && ns.getServerRequiredHackingLevel(servers[i]) < ns.getHackingLevel()) {
	...
	function updateServersRam() {
		...
		for (let i = 0; i < serversObjectArray.length; i++) {
			...
			if (serversObjectArray[i].name == "home") serversObjectArray[i].freeRam -= 12;
		}
