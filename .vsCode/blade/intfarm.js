...
export async function main(ns) {
    ...
    function enableJob(operation, amount = 1000) {
        if (operation == "Assassination") amount = 10000;
        if (ns.bladeburner.getActionCountRemaining("Operations", operation) > amount)
            jobs[operation] = true;
        if (ns.bladeburner.getActionCountRemaining("Operations", operation) <= 10)
