...
export async function main(ns, homeReservedRam = 16) {
	...
	const cores = 1;
	const filterServ = 200; //remove servers with more than this hack skill needed
	const moneyThresh = 0.7; //don't hack if money is less than this %
	const weakThresh = 1.3; //don't hack if security is more than this %
	const moneyToSteal = 0.25;
	const spamDifficulty = 0; //Spam scripts if required hacking level is less than this
	...
	*				@runHack: threads
	...
	for (let i = 0; i < targets.length; i++) {
		...
		if (ns.hasRootAccess(targets[i])) allTargetObjects.push(tempObject);
	}
	filterTargets(filterServ); //remove target if weaktime is more than this many seconds
	...
	function filterTargets(time) {
		...
		for (let i = allTargetObjects.length - 1; i >= 0; i--) {
			if (ns.getServerRequiredHackingLevel(allTargetObjects[i].name) > time)
	...
	function updateTail() {
		...
		ns.print("       ├───────────────┼───────┼───────┼───┼───┼───┤");
		//	ns.print("        zb-def          $100%, sec: 102% T: 1");
	...
	async function runThreads() {
		let tempTotalRam = 0;
		objectArraySort(serversQueue, "runGrow", "small");
		for (let serv of serverObjects) {
			...
			if (toGrow > 0) { // && tempTotalRam > tempWeakMem + tempGrowMem + tempHackMem) {
				for (let serv of serverObjects) {
					...
					if (tempGrowMem < serv.freeRam) {
						...
						if (!findRunning(targ.name, growFile) || (ns.getServerRequiredHackingLevel(targ.name) < spamDifficulty)) serversQueue.push(tempServerObject);
						tempGrowMem = 0;
						break;
					} else {
			...
			if (toWeak > 0) { // && tempTotalRam > tempWeakMem + tempGrowMem + tempHackMem) {
				for (let serv of serverObjects) {
					...
					if (tempWeakMem < serv.freeRam) {
						...
						if (!findRunning(targ.name, weakFile) || (ns.getServerRequiredHackingLevel(targ.name) < spamDifficulty)) serversQueue.push(tempServerObject);
						tempWeakMem = 0;
						break;
					} else {
			...
			if (toHack > 0) { // && tempTotalRam > tempWeakMem + tempGrowMem + tempHackMem) {
				for (let serv of serverObjects) {
					...
					if (tempHackMem < serv.freeRam) {
						...
						if (!findRunning(targ.name, hackFile) || (ns.getServerRequiredHackingLevel(targ.name) < spamDifficulty)) serversQueue.push(tempServerObject);
						tempHackMem = 0;
						break;
					} else {
	...
	function getTimers(thisTarget) {
		...
		thisTarget.hackTimeOffset = thisTarget.weakingTime - thisTarget.hackingTime + 100;
	}
	...
	function getThreads() {
		for (var target of validTargetObjects) {
			let tempAvail = ns.getServerMoneyAvailable(target.name);
			if ((ns.getServerMoneyAvailable(target.name) == 0)) tempAvail = ns.getServerMaxMoney(target.name) * 0.1; //avail not less than 10% to prevent div/0 error
			target.growThreadsNeeded =
				Math.ceil(
					ns.growthAnalyze(
						target.name,
						(ns.getServerMaxMoney(target.name) / tempAvail) * (1.02 + moneyToSteal), //1.02 safety multiplier
						cores
					)
				);
