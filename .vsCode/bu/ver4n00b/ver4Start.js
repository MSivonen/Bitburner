...
export async function main(ns, homeReservedRam = 16) {
	...
	const cores=1;
	const filterServ = 2; //remove servers with more than this hack skill needed
	const moneyThresh = 0.7;
	const weakThresh = 1.3;
	const moneytoSteal = 0.2;
	const spamDifficulty = 10; //Spam scripts if required hacking level is less than this
	...
	*				@runHack: threads
	...
	function updateTail() {
		...
		ns.print("Free RAM: " + tempFreeRam.toFixed(0) + gb + " / " + tempTotalRam.toFixed(0) + gb + "  " + perce.toFixed(0) + "%");
	}
