...
export async function main(ns) {
    ...
    await writeToJSON(ns, data, "corpservers.txt");

    /*let symbols = ns.stock.getSymbols();
    for (let sym of symbols) {
        let match = data.find(s => s.sym == sym);
        if (match == undefined)
            ns.tprint('WARN: ' + sym.padEnd(10) + ' : ' + 'No match!'.padEnd(20) + '???');
        else
            ns.tprint('INFO: ' + match.sym.padEnd(10) + ' : ' + match.server.padEnd(20) + match.organization);
    }*/
