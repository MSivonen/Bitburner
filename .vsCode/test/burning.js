...
export async function main(ns) {
	...
	class Particle {
		...
		constructor(x_, y_, xs_, ys_) {
			...
			this.vel.x += Math.random() * 8;
			this.vel.y += Math.random() * 8;
		}
	...
	while (true) {
		...
		for (let i = 0; i < TWO_PI; i = i + TWO_PI / 100) {
			...
			i = i + phase;
			let x = pluspos + 4 * Math.sin(i);
			let y = pluspos + 4 * Math.cos(i);
			...
			if (randomInt(0, 10) < 6) {
				particleA.push(new Particle(x, y, windSpeed + 1, randomInt(-6, -10) / 5));
			}
			i = i - phase;
		}
		ctx.fillStyle = "rgba(80, 60, 30, 1)";
		ctx.stroke(); //path
		ctx.fill(); //path
		phase += .01;
		pluspos++;
