...
export async function main(ns, homeReservedRam = 32) {
	...
	const spamDifficulty = 1800; //Spam scripts if required hacking level is less than this
	const shareServers = 7;
	...
	*				@runHack: threads
	...
	function giveJobs() {
		...
		for (let targ of validTargetObjects) {
			...
			for (let serv of serverObjects) {
				tempTotalRam += (serv.freeRam) * .9; //10% safety reserve ram
			}
	...
	async function updateServers() {
		...
		for (let i = servers.length; i >= 0; i--) {
			try {
				const [fullMatch, serverNumber] = servers[i].match(/perkele(\d+)/);
